...
class _AdminProductDetailScreenState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final images = await service.getProductImages(
        _galleryId,
        widget.productId,
      );
      if (mounted)
      ...
      if (mounted)
        setState(() {
          _error =
              'No se pudo cargar el producto. Verifica tu conexión e intenta de nuevo.';
          _loading = false;
        });
    }
  }
  ...
  Future<void> _deleteProduct() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          '¿Eliminar "${_product?.name ?? 'este producto'}"? Esta acción no se puede deshacer.',
        ),
        actions: [
          TextButton(
    ...
    try {
      await ref
          ...
          .deleteProduct(_galleryId, widget.productId);
          ...
          if (p.imageUrl.isNotEmpty) ...[
            ClipRRect(
              ...
              child: SizedBox(
                ...
                child: AppImage('$_baseUrl${p.imageUrl}', cacheWidth: 1000),
              ),
            ),
            ...
            children: [
              Expanded(
                child: _InfoCard(
                  ...
                  label: 'Precio',
                  value: '\$${(p.basePrice / 100).toStringAsFixed(2)} USD',
                ),
              ),
              ...
              Expanded(
                child: _InfoCard(
                  icon: Icons.percent,
                  label: 'Comisión',
                  value: '${p.commissionPercent.toStringAsFixed(1)}%',
                ),
              ),
            ],
          ),
            ...
            children: [
              ...
              Text(
                ...
                style: tt.labelMedium?.copyWith(
                  color: _images.length >= 5 ? cs.error : cs.onSurfaceVariant,
                ),
              ),
            ],
          ),
                  ...
                  children: [
                    ClipRRect(
                      ...
                      child: SizedBox(
                        ...
                        height: 100,
                        child: AppImage(
                          imageVariant('$_baseUrl${_getUrl(img)}', 'thumb'),
                          fallbackUrl: '$_baseUrl${_getUrl(img)}',
                          cacheWidth: 250,
                          errorIconSize: 24,
                        ),
                      ),
